...
const loadIcons = () => {
  try {
    ...
    return Array.isArray(parsed) ? parsed.filter(x => x?.id && x?.dataUrl).map(icon => ({
      ...icon,
...
const setupButtons = () => {
  ...
  if (addBtn) {
    addBtn.addEventListener("click", () => {
      ...
      // Reset form
      const inputs = ["custom-icon-file", "custom-icon-name", "custom-icon-is-camera", 
                     "custom-icon-cctv", "custom-icon-access", "custom-icon-intruder", "custom-icon-fire"];
      inputs.forEach(id => {
  ...
  if (saveBtn) {
    saveBtn.addEventListener("click", async () => {
      ...
      const sections = [];
      ["custom-icon-cctv", "custom-icon-access", "custom-icon-intruder", "custom-icon-fire", "custom-icon-custom"]
        .forEach(id => {
          const el = document.getElementById(id);
