...
function initTopologyBuilder(mainCanvas) {
  ...
  const devices = allDevices.filter(dev => {
    const deviceId = topologyMgr && typeof topologyMgr.getDeviceId === 'function' 
      ? topologyMgr.getDeviceId(dev) 
      : getStableId(dev);
  ...
  const SAFE_MARGIN_BOTTOM = Math.max(24, NODE_RADIUS + LABEL_OFFSET + LABEL_HEIGHT + 8);
    ...
    devices.forEach((dev) => {
      const id = topologyMgr && typeof topologyMgr.getDeviceId === 'function' 
        ? topologyMgr.getDeviceId(dev) 
        : getStableId(dev);
  ...
  function renderAllFromWorking() {
    ...
    workingConnections.forEach((wc) => {
      ...
      const pairKey = wc.device1Id < wc.device2Id ? 
        `${wc.device1Id}_${wc.device2Id}` : 
        `${wc.device2Id}_${wc.device1Id}`;
  ...
  function renderMultipleConnections(canvas, connections, registry) {
    ...
    connections.forEach((connection, index) => {
      ...
      const line = new fabric.Line([
        lineStart.x + offset, lineStart.y,
        lineEnd.x + offset, lineEnd.y
      ], {
        ...styles.line,
        selectable: false,
        hasControls: false,
